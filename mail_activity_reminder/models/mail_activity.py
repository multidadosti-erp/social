...
class MailActivity(models.Model):
    ...
    def action_remind(self):
        ...
        for activity in self:
            ...
            # Envia Msg
            channel_id = request.env['mail.channel'].search([
                ('name', '=', activity.user_id.name + ", " + request.env.user.name)
            ])

            if not channel_id:
                channel_dict = request.env['mail.channel'].channel_get([activity.user_id.partner_id.id])
                if channel_dict['id']:
            ...
            body = _('<p> <b>Activity Type : </b><i class="fa {4}" style="color:red;"/> "{0}"</p> <b>Date : </b>{1}<p><b>Assigned'\
                ...
                '</a></b> </br>').format(activity.activity_type_id.name, activity.date_deadline, activity.res_name, url, activity.icon)
